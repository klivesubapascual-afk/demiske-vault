...
exports.handler = async function (event) {
  try {
    ...
    if (!account_id || !seller || !customer_telegram_id || !payment_reference || !proof_image_base64) {
      return jsonResponse(400, { error: "Missing account, seller, Telegram ID, reference number, or proof screenshot." });
    }
    ...
    if (accountError || !account) return jsonResponse(404, { error: "Account not found." });
    ...
    const caption =
      ...
      `<b>Customer Telegram ID:</b> ${escapeHtml(customer_telegram_id)}\n` +
    ...
    const tgResponse = await fetch(TELEGRAM_API(botToken, "sendPhoto"), { method: "POST", body: formData });
    const tgData = await tgResponse.json();

    if (!tgData.ok) return jsonResponse(500, { error: "Telegram send failed.", details: tgData.description });

    return jsonResponse(200, { success: true, message: "Payment proof sent to admin for approval.", order_id: order.id });
  } catch (error) {
