...
exports.handler = async function (event) {
  try {
    if (event.httpMethod !== "POST") return jsonResponse(200, { ok: true });
    ...
    if (!update.callback_query) return jsonResponse(200, { ok: true });
    ...
    if (!["approve", "deny"].includes(action) || !orderId) {
      await telegram("answerCallbackQuery", { callback_query_id: callback.id, text: "Invalid action." });
      return jsonResponse(200, { ok: true });
    }
    ...
    if (orderError || !order) {
      await telegram("answerCallbackQuery", { callback_query_id: callback.id, text: "Order not found.", show_alert: true });
      return jsonResponse(200, { ok: true });
    }

    const { data: account, error: accountError } = await supabase.from("accounts").select("*").eq("id", order.account_id).single();

    if (accountError || !account) {
      await telegram("answerCallbackQuery", { callback_query_id: callback.id, text: "Account not found.", show_alert: true });
      return jsonResponse(200, { ok: true });
    }
    ...
    if (action === "approve") {
      ...
      await telegram("sendMessage", { chat_id: order.customer_telegram_id, text: customerMessage, parse_mode: "HTML" });
    ...
    if (action === "deny") {
      await supabase.from("orders").update({ status: "denied" }).eq("id", orderId);
      ...
      await telegram("answerCallbackQuery", { callback_query_id: callback.id, text: "Denied. Customer has been notified." });
